...
func (h *AIHandler) Chat(c *gin.Context) {
	...
	userType, exists2 := c.Get("userType")
	if !exists1 || !exists2 {
	...
	userTypeStr := fmt.Sprintf("%v", userType)
	...
	// 5. 调用service或许数据流channel
	// 将gin的请求上下文传递给service
	streamChan, err := h.aiService.ChatStream(c.Request.Context(), req.Query, userIDStr, userTypeStr)
	if err != nil {
		// 通常是服务启动前的配置问题
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": "<UNK>", "data": nil})
		return
	}
	...
	for {
		...
		case <-c.Request.Context().Done():
			fmt.Println("Client disconnected 客户端已断开连接")
			return
		case resp, ok := <-streamChan:
			...
			if resp.Err != nil {
				// 流式处理发生的错误
				fmt.Println("流失处理错误：%v\n", resp.Err)
				errorMsg := gin.H{"code": 500, "msg": resp.Err.Error()}
				errJSON, _ := json.Marshal(errorMsg)
				c.Writer.WriteString("data:" + string(errJSON) + "\n\n")
				c.Writer.Flush()
				return
			}
			// 获取数据流，写入响应流中
			c.Writer.WriteString("data: " + resp.Content + "\n\n")
			c.Writer.Flush()
		}
	}
}
